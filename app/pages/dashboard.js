  ...
  const fetchUserDetails = async () => {
    try {
      const { data, error } = await supabase
        .from('user_details')
        .select(
          'user_id, username, first_name, last_name, address, phone_number, gender, profile_picture, language, biography'
        )
        .eq('user_id', user.id)
        .single();
        ...
        {Object.keys(userDetails).length > 0 && (
          <table className="w-full">
            <tbody>
              ...
              <tr>
                <td className="py-2 font-semibold">Biography:</td>
      ...
      {showForm && Object.keys(userDetails).length > 0 && (
        <div className="max-w-lg mx-auto  border rounded shadow-sm">
          ...
          <input
            ...
            value={userDetails ? userDetails.biography || '' : ''}
            onChange={handleInputChange}
          />
